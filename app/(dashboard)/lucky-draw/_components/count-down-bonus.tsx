...
const useCountdown = () => {
  // Calculate the next draw date (Sunday 9 PM SGT)
  const calculateNextDrawDate = () => {
    ...
    if (currentDay === 0) {
      // Check if it's past 9 PM SGT
      const drawTime = now.hour(21).minute(0).second(0).millisecond(0);
      if (now.isAfter(drawTime)) {
        // Move to next Sunday
        nextSunday = now.add(7, "day");
      } else {
        // Use today (Sunday) at 9 PM
        nextSunday = drawTime;
      }
    } else {
      // Calculate days until next Sunday
      const daysUntilSunday = 7 - currentDay;
      nextSunday = now.add(daysUntilSunday, "day");
    }

    // Set to 9 PM SGT
    nextSunday = nextSunday.hour(21).minute(0).second(0).millisecond(0);

    return nextSunday.toDate().getTime();
  };
...
export default CountDownBonusDraw;
