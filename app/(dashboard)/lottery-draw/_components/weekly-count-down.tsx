...
const useCountdown = () => {
  // Calculate the next draw date - now adjusted to next Sunday
  const calculateNextDrawDate = () => {
    const now = dayjs().tz("Asia/Singapore");
    let nextDrawDate = dayjs()
      .tz("Asia/Singapore")
      .hour(19)
      .minute(30)
      .second(0)
      .millisecond(0); // Set to 19:30 SGT

    const currentDay = now.day(); // 0 is Sunday, 1 is Monday, etc.

    if (currentDay === 0) {
      // Today is Sunday
      if (now.isAfter(nextDrawDate)) {
        // If current time is past 19:30 SGT on Sunday, set to next Sunday
        nextDrawDate = nextDrawDate.add(7, "day");
      }
    } else {
      // Not Sunday, calculate days until next Sunday
      const daysUntilSunday = 7 - currentDay;
      nextDrawDate = nextDrawDate.add(daysUntilSunday, "day");
    }

    return nextDrawDate.toDate().getTime();
  };
