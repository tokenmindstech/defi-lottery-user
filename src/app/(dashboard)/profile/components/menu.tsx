...
const ProfileMenu = () => {
  ...
  return (
    <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-5">
      <div className="flex h-fit flex-col col-span-3 lg:col-span-1 space-y-5 bg-bgtext-900 rounded-xl px-4 py-4">
        {PROFILE_MENU_ITEMS.map((item, idx) => (
          <Button
            key={idx}
            onClick={() => setActiveTab(item.value)}
            className={cn(
              "w-full bg-bgtext-900 border-0 hover:bg-gradient-to-r from-linviolet-start to-transparent hover:text-bgtext-100 ease-out transition-all duration-300 rounded-lg cursor-pointer",
